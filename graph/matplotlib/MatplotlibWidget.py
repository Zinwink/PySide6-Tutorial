...
"""
    ...
    从Matplotlib的Qt后端导入FigureCanvasQTAgg类，并给它起别名FigureCanvas
    FigureCanvasQTAgg是一个继承自Qt Widget的类，它可以将Matplotlib图形嵌入到PyQt应用程序中
    这个类负责将Matplotlib的图形渲染到Qt的绘图表面

    从Matplotlib的Qt后端导入NavigationToolbar2QT类，并给它起别名NavigationToolbar
    这个类提供了一个工具栏，包含常用的图形操作按钮，如缩放、平移、保存等
    它是专门为Qt应用程序设计的Matplotlib导航工具栏
