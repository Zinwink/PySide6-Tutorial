...
class MainWindow(QMainWindow):
    ...
    """
    active_mdi_child()
    返回当前活动的MDI子窗口（QMdiSubWindow）或None
    在多文档界面中确定用户当前正在操作的窗口
    copy():执行标准的复制操作，将选中内容复制到剪贴板
    """
    def cut(self):
    ...
    def read_settings(self):
        ...
        # 读取键 geometry的值 默认为QByteArray
        geometry = settings.value('geometry', QByteArray())
        # 恢复geometry 包括窗口位置、大小和状态
        if geometry.size():
            self.restoreGeometry(geometry)

    def write_settings(self):
        # 保存配置
        # 存储位置​​：不同平台存储位置不同，但QSettings会自动处理
        settings = QSettings('QtProject', 'MDI Example')
    ...
    def find_mdi_child(self, fileName):
        # QFileInfo是Qt处理文件信息的类，canonicalFilePath()返回文件的绝对规范路径
