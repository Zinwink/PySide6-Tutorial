...
class filedialogdemo(QWidget):
    ...
    def file_pic_filter1(self):
        # filter中 ";;" 用于分割文件类型选择项
        fname, _ = QFileDialog.getOpenFileName(self,caption="Open file1",dir=os.path.abspath('.')+'\\images',filter="Images files (*.jpg *.png);;Images files2(*.ico *.gif);;All files(*)")
        self.label.setPixmap(QPixmap(fname))
        self.label2.setText('你选择了:\n' + fname)
    
    def file_pic_filter2(self):
        file_dialog = QFileDialog(self, caption="Open file2",directory=os.path.abspath('.')+'\\images',filter="Image files (*.jpg *.png);;Image files2(*.ico *.gif);;All files(*)")
