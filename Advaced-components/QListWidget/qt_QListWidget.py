...
项目
(Item)是组成列表的基本单位,每个项目都是 QListWidgetItem 类的实例
QListWidget包含内部模型,并通过内部模型管理 QListWidgetItem。
QListWidget 适用于显示简单的列表,
如果想要更强大的列表显示功能,则使用 QListView。QListView 可以使用自定义模型,而 QListWidget 只能使用内部模型。

QListWidget 是 QListView的子类,可以看作 QListView 的简单化操作子类,集成了内部模型,并通过 QListWidgetItem
来管理项目
QListWidget.takeItem(row)函数可以删除项目
QListWidgetItem.flag()函数决定了用户能否对项目进行选择、编辑及交互
QListWidget.selectionMode()函数决定了在列表中可以同时选择多少个项目,以及是否
可以创建复杂的项目选择,这可以通过 setSelectionMode()函数进行设置

QListWidget.selectionBehavior()函数决定了用户的选择行为,同时决定用户选择单个
项目、行还是列,可以通过 setSelectionBehavior()修改函数值
"""
...
class QListWidgetDemo(QMainWindow):
    ...
    def __init__(self, parent= None):
        ...
        tool_button.setIcon(QIcon("./help.jpg"))
    ...
    def generateMenu(self):
        ...
        menu.addAction(QIcon("./close.png"),'删除',self.onDelete,QKeySequence(Qt.CTRL|Qt.Key_D))
        menu.addSeparator()
